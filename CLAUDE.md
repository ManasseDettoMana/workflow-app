...
```powershell
python -m pip install -r requirements-build.txt # PyInstaller; not in the dev set
python tools\make_icon.py                       # redraw app.ico (committed; rarely needed)
.\tools\build.ps1                               # both artefacts, into dist\
.\tools\build.ps1 -SkipIcon                     # without redrawing the icon
.\tools\install-shortcut.ps1                    # Desktop and Start Menu icons
.\tools\install-shortcut.ps1 -Remove            # and take them off again
```
...
workflowapp/gui/      depends on core, never the reverse
  ...
  themes/
    light.qss, dark.qss
  assets/
    app.ico           six frames, 16 to 256; generated by tools/make_icon.py
packaging/            the Windows build
  workflowapp.spec    one Analysis, two EXE targets: the folder build and the portable one
  entry.py            the script PyInstaller freezes
  file_version_info.py  the exe's version resource, built from workflowapp.__version__
tools/                make_icon.py, build.ps1, install-shortcut.ps1, install-hooks.ps1
...
- **The `.qss` files and `app.ico` must stay declared in `[tool.setuptools.package-data]`**, and in
  `datas` in `packaging/workflowapp.spec`. Left out, they load fine from a source checkout and are
  missing from an installed or frozen build, where the application starts unstyled with nothing to
  say why. `tests/test_packaging.py` asserts the declaration and `tools/build.ps1` asserts the
  files are in the bundle, because neither failure is visible from the running application.
- **Runtime resources are reached with `importlib.resources`, never a path built from `__file__`.**
  `gui/theme.py` and `gui/assets.py` are the only two places that load one, and both go through
...
- **The frozen executable shares `QSettings` with the source-run application** - same organisation
  and application name, so the same registry key. The theme and window position carry across
  between `python -m workflowapp` and the exe, which is usually wanted and is occasionally
